...
updatesystray(void)
{
	...
	if (!systray) {
		...
		XChangeProperty(dpy, systray->win, netatom[NetSystemTray], XA_ATOM, 32,
			PropModeReplace, (unsigned char *)&netatom[NetSystemTray], 1);
		...
		if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
			...
			ev.type = ClientMessage;
			ev.window = root;
			ev.message_type = netatom[NetSystemTray];
			ev.format = 32;
			ev.data.l[0] = CurrentTime;
			ev.data.l[1] = netatom[NetSystemTray];
			ev.data.l[2] = systray->win;
			ev.data.l[3] = 0;
			ev.data.l[4] = 0;
	...
	for (i = systray->icons; i; i = i->next) {
		/* skip icons that are not mapped */
		if (!i->tags)
			continue;
		if (i->mon != selmon)
			i->mon = selmon;
